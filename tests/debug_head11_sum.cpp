...
int main() {
    ...
    // Focus on head 11 for token 9, output 410
    int t = 9, o = 410, h = 11;
    fprintf(stderr, "=== Head %d contribution to matmul at (t=%d, o=%d) ===\n\n", h, t, o);

    float sum_ggml = 0, sum_ref = 0;
    fprintf(stderr, "First 10 products:\n");
    for (int i = 0; i < d_head; i++) {
        int k = h * d_head + i;
        float w = weight[k + o * d_model];
        float a_ggml = got_attn[k + t * d_model];
        float a_ref = ref_attn_out[t * d_model + k];
        float prod_ggml = w * a_ggml;
        float prod_ref = w * a_ref;
        sum_ggml += prod_ggml;
        sum_ref += prod_ref;

        if (i < 10) {
            fprintf(stderr, "  i=%2d: w=%+.4f, a_ggml=%+.4f, a_ref=%+.4f, prod_ggml=%+.6f, prod_ref=%+.6f, diff=%+.6f\n",
                    i, w, a_ggml, a_ref, prod_ggml, prod_ref, prod_ggml - prod_ref);
        }
    }

    fprintf(stderr, "\nLast 10 products (i=54-63):\n");
    sum_ggml = sum_ref = 0;  // Reset for recalculation
    for (int i = 0; i < d_head; i++) {
        int k = h * d_head + i;
        float w = weight[k + o * d_model];
        float a_ggml = got_attn[k + t * d_model];
    ...
    fprintf(stderr, "\nFinal head %d sum: GGML=%+.6f, ref=%+.6f, diff=%+.6f\n",
            h, sum_ggml, sum_ref, sum_ggml - sum_ref);

    // Also check absolute values of attention for head 11
    fprintf(stderr, "\nSum of absolute attention values for head %d, token %d:\n", h, t);
    float abs_sum_ggml = 0, abs_sum_ref = 0;
    for (int i = 0; i < d_head; i++) {
        int k = h * d_head + i;
        abs_sum_ggml += std::fabs(got_attn[k + t * d_model]);
        abs_sum_ref += std::fabs(ref_attn_out[t * d_model + k]);
    }
    fprintf(stderr, "  GGML=%.4f, ref=%.4f\n", abs_sum_ggml, abs_sum_ref);
