...
int main() {
    ...
    // Trace the first 10 weight*attn terms for position (t=9, o=410)
    int t = 9, o = 410;
    fprintf(stderr, "Tracing weight*attn products for t=%d, o=%d:\n", t, o);
    ...
    for (int k = 0; k < 20; k++) {
        ...
        sum_ggml += prod_ggml;
        sum_ref += prod_ref;
        fprintf(stderr, "  k=%2d: w=%.4f, a_ggml=%.4f, a_ref=%.4f, prod_ggml=%.4f, prod_ref=%.4f\n",
                k, w, a_ggml, a_ref, prod_ggml, prod_ref);
    }
    fprintf(stderr, "  (partial sum after 20): GGML=%.4f, ref=%.4f\n", sum_ggml, sum_ref);

    // Complete the sum
    for (int k = 20; k < d_model; k++) {
        float w = weight[k + o * d_model];
        float a_ggml = got_attn[k + t * d_model];
        float a_ref = ref_attn_out[t * d_model + k];
        sum_ggml += w * a_ggml;
    ...
    fprintf(stderr, "\nFinal sums: GGML=%.6f, ref=%.6f, ratio=%.3f\n", sum_ggml, sum_ref, sum_ggml / sum_ref);
