...
int main() {
    ...
    // Now compute sa_out manually using GGML attention output
    int t = 9, o = 410;
    float sum_ggml = 0, sum_ref = 0;
    for (int k = 0; k < d_model; k++) {
        float w = weight[k + o * d_model];  // W_ggml[k, o]
        float a_ggml = got_attn[k + t * d_model];  // A_ggml[k, t]
        float a_ref = ref_attn_out[t * d_model + k];  // PyTorch ref
        sum_ggml += w * a_ggml;
        sum_ref += w * a_ref;
    }

    fprintf(stderr, "Manual matmul at position (t=%d, o=%d):\n", t, o);
    fprintf(stderr, "  Using GGML attn: %.6f\n", sum_ggml);
    fprintf(stderr, "  Using PyTorch ref: %.6f\n", sum_ref);
    fprintf(stderr, "  Expected: 0.246909\n");
    fprintf(stderr, "  Ratio GGML/ref: %.3f\n", sum_ggml / sum_ref);

    // Let's also check what values differ
    fprintf(stderr, "\nComparing attention outputs (first few with large diff):\n");
    int count = 0;
    for (int i = 0; i < d_model * seq_len && count < 20; i++) {
        float diff = std::fabs(got_attn[i] - ref_attn_out[i]);
        if (diff > 0.1) {
            int tt = i / d_model;
            int dd = i % d_model;
            fprintf(stderr, "  [t=%d, d=%d]: GGML=%.4f, ref=%.4f, diff=%.4f\n",
                    tt, dd, got_attn[i], ref_attn_out[i], diff);
            count++;
        }
    }

    // Check row sums for token 9
    fprintf(stderr, "\nRow sums for token 9:\n");
    float row_sum_ggml = 0, row_sum_ref = 0;
    for (int d = 0; d < d_model; d++) {
        row_sum_ggml += got_attn[d + 9 * d_model];
        row_sum_ref += ref_attn_out[9 * d_model + d];
    }
    fprintf(stderr, "  GGML sum: %.6f\n", row_sum_ggml);
    fprintf(stderr, "  Ref sum: %.6f\n", row_sum_ref);
    fprintf(stderr, "  Ratio: %.3f\n", row_sum_ggml / row_sum_ref);
