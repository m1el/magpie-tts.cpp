...
int main(int argc, char ** argv) {
    ...
    const char * expected_path = "test_data/reference/manual_enc_layer0_out.bin";
    ...
    if (!layer->norm_self_w || !layer->sa_qkv_w || !layer->sa_out_w ||
        ...
        fprintf(stderr, "Encoder layer 0 weights not loaded!\n");
        magpie_free(ctx);
        return 1;
    }

    fprintf(stderr, "Encoder layer 0 weights:\n");
    fprintf(stderr, "  norm_self_w: [%lld]\n", (long long)layer->norm_self_w->ne[0]);
    fprintf(stderr, "  sa_qkv_w: [%lld, %lld]\n", (long long)layer->sa_qkv_w->ne[0], (long long)layer->sa_qkv_w->ne[1]);
    fprintf(stderr, "  sa_out_w: [%lld, %lld]\n", (long long)layer->sa_out_w->ne[0], (long long)layer->sa_out_w->ne[1]);
    fprintf(stderr, "  ff_proj_w: [%lld, %lld, %lld]\n", (long long)layer->ff_proj_w->ne[0],
            (long long)layer->ff_proj_w->ne[1], (long long)layer->ff_proj_w->ne[2]);
    fprintf(stderr, "  ff_out_w: [%lld, %lld, %lld]\n", (long long)layer->ff_out_w->ne[0],
            (long long)layer->ff_out_w->ne[1], (long long)layer->ff_out_w->ne[2]);
    ...
    struct ggml_tensor * output = magpie_build_encoder_layer(
        ctx0, input_tensor, nullptr, layer, &ctx->model.hparams);

    if (!output) {
        fprintf(stderr, "magpie_build_encoder_layer returned null!\n");
        ggml_free(ctx0);
        magpie_free(ctx);
        return 1;
    }
    ...
    if (mask) {
        ...
        fprintf(stderr, "Note: No causal_mask tensor found\n");
    }
    ...
    // Compare with relaxed tolerance for accumulated FP errors
    fprintf(stderr, "\nComparing results:\n");
    ...
    if (match) {
        fprintf(stderr, "\nSUCCESS: Encoder layer matches reference!\n");
        return 0;
    } else {
        fprintf(stderr, "\nFAILURE: Encoder layer does not match reference\n");
        return 1;
    }
}
