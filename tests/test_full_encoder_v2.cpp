// Test full encoder (6 layers) against PyTorch reference
...
int main() {
    ...
    // Load text embeddings (before position embeddings)
    int64_t in_shape[4];
    auto input_data = load_reference("test_data/reference/text_embedded.bin", in_shape);

    // Load expected final encoder output
    int64_t out_shape[4];
    auto expected = load_reference("test_data/reference/enc_output.bin", out_shape);
    ...
    // Build graph
    size_t ctx_size = ggml_tensor_overhead() * 512 + 64 * 1024 * 1024;
    ...
    struct ggml_tensor * output = magpie_build_full_encoder(ctx0, input, &mctx->model.encoder, &hp);
    ...
    printf("\nFull Encoder comparison (6 layers + final norm):\n");
    ...
    if (max_diff < 1.0f) {
        printf("\nSUCCESS: Full encoder matches within tolerance!\n");
        return 0;
    } else {
        printf("\nFAILURE: Full encoder has large errors\n");
        return 1;
    }
}
