// Test self-attention step against PyTorch reference
...
int main() {
    ...
    // Load norm output (input to attention)
    int64_t in_shape[4];
    auto input_data = load_reference("test_data/reference/enc_l0_norm1.bin", in_shape);

    // Load expected attention output
    int64_t out_shape[4];
    auto expected = load_reference("test_data/reference/enc_l0_attn.bin", out_shape);
    ...
    // Build graph
    size_t ctx_size = ggml_tensor_overhead() * 64 + 16 * 1024 * 1024;
    ...
    struct ggml_tensor * output = magpie_build_self_attention(
        ctx0, input, layer.sa_qkv_w, layer.sa_out_w, hp.enc_heads, true);
    ...
    if (max_diff < 0.01f) {
        printf("SUCCESS: Attention matches!\n");
        return 0;
    } else {
        printf("FAILURE: Attention mismatch\n");
        return 1;
    }
}
